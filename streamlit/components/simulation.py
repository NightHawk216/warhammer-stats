"""Simluation Folder for the Warhammer Stats App."""
...
def run_simulation_logic(inputs: t.Dict[str, t.Any], the_type: str) -> None:
    """
    Run the simulation logic for Warhammer stats calculations.
    ...
    inputs : dict
        ...
        - total_number_of_dice_rolls : int
            Total number of dice rolls to simulate.
    ...
    the_type : str
        The type of calculation (e.g., "Hits", "Wounds").

    Returns
    -------
    None
        This function does not return a value. It updates the Streamlit session state and displays results.
    """
    ...
    number_of_runs = st.number_input(
        "Number of Runs", min_value=1, max_value=10000, value=20, step=1
    )
            ...
            for attacker in inputs["attacker_stats"]:
                if the_type == "Hits":
                    ...
                    if number_of_attacks:
                        num_success_rolls, roll_list = multiple_hits(
                            attackers_weapon_skill, defenders_stats, number_of_attacks
                        )
                ...
                if the_type == "Wounds":
                    ...
                    number_of_attacks = attacker["number_of_attacks"]
                    if number_of_attacks:
                        num_success_rolls, roll_list = multiple_wounds(
                            attacker_strength, defenders_stats, number_of_attacks
                        )
                        total_successful_rolls.append(num_success_rolls)
                        total_roll_list.append(roll_list)
                        total_successes += num_success_rolls
        ...
        if show_labels:
            ...
            current_color = random.choice(
                [
                    ...
                    if color != st.session_state["last_graph_color"]
                ]
            )
            ...
            fig = px.histogram(
                df,
                x=f"Number of {the_type}",
                nbins=20,
                title=f"Simulation Results: Number of {the_type}",
            )
            fig.update_traces(
                marker=dict(color=current_color, line=dict(color="black", width=1))
            )
            fig.update_layout(
                xaxis_title=f"Number of {the_type}", yaxis_title="Frequency"
            )
            ...
            previous_results = (
                ...
                if st.session_state["last_simulation_results"]
                else []
            )
            previous_color = (
                st.session_state["last_simulation_results"]["color"]
                if st.session_state["last_simulation_results"]
                else "gray"
            )
            ...
            if previous_results:
                ...
                prev_fig = px.histogram(
                    ...
                    x=f"Number of {the_type}",
                    nbins=20,
                    title=f"Previous Simulation Results: Number of {the_type}",
                )
                prev_fig.update_traces(
                    marker=dict(color=previous_color, line=dict(color="black", width=1))
                )
                ...
                prev_counts = (
                    prev_df[f"Number of {the_type}"].value_counts().sort_index()
                )
            ...
            st.session_state["last_simulation_results"] = {
                "results": results,
                "color": current_color,
            }
