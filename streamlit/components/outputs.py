"""Output component for Warhammer stats calculator."""
...
def render_outputs(inputs: dict) -> None:
    """
    Render the outputs for the Warhammer stats calculator based on the provided inputs.
    ...
    if num_attackers:
        if st.button("Calculate Hits"):
            ...
            for attacker in inputs["attacker_stats"]:
                ...
                num_success_hit_rolls = number_of_attacks = attacker[
                    "number_of_attacks"
                ]
                ...
                if number_of_attacks and attacker_weapon_skill:
                    num_success_hit_rolls, hit_roll_list = multiple_hits(
                        attacker_weapon_skill,
                        defender_weapon_skill,
                        number_of_attacks,
                        reroll_criteria=hit_rerolls,
                    )
                ...
                if num_success_hit_rolls and attacker_strength:
                    ...
                    num_success_wounds_rolls, wounds_roll_list = multiple_wounds(
                        attacker_strength,
                        defender_toughness,
                        num_success_hit_rolls,
                        reroll_criteria=wound_rerolls,
                    )
            ...
            for idx in range(num_attackers):
                ...
                if the_type in ["Combat", "Hits"]:
                    ...
                    for entry in total_hit_roll_list[idx]:
                        if entry["result"] == "Hit":
                            first_rolls.append(
                                (
                                    entry["first_roll"],
                                    "<span style='color:green; font-weight:bold;'>{}</span>".format(
                                        entry["first_roll"]
                                    ),
                                )
                            )
                            ...
                            entry["result"] == "Miss"
                            and entry["second_roll"] is not None
                        ):
                            first_rolls.append(
                                (
                                    entry["first_roll"],
                            ...
                            first_rolls.append(
                                (
                                    entry["first_roll"],
                                    "<span style='color:black;'>{}</span>".format(
                                        entry["first_roll"]
                                    ),
                                )
                            )
                    ...
                    highlighted_first_rolls = [
                        highlight for _, highlight in first_rolls
                    ]
                    ...
                    highlighted_second_rolls = [
                        highlight for _, highlight in second_rolls
                    ]
            ...
            if the_type in ["Hits"]:
                if total_num_wounds == 1:
                    st.markdown(
                        f"<h1 style='font-size:100px; text-align:center;'>{total_num_hits} Hit</h1>",
                        unsafe_allow_html=True,
                    )
                else:
                    st.markdown(
                        f"<h1 style='font-size:100px; text-align:center;'>{total_num_hits} Hits</h1>",
                        unsafe_allow_html=True,
                    )
            ...
            if the_type in ["Combat", "Wounds"]:
                ...
                if total_num_wounds == 1:
                    st.markdown(
                        f"<h1 style='font-size:100px; text-align:center;'>{total_num_wounds} Wound</h1>",
                        unsafe_allow_html=True,
                    )
                else:
                    st.markdown(
                        f"<h1 style='font-size:100px; text-align:center;'>{total_num_wounds} Wounds</h1>",
                        unsafe_allow_html=True,
                    )
